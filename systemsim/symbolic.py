...
def make_C(M, q, q_dot):
    ...
    # Standard way of deriving C matrix from the mass matrix M (See e.g. van der Schaft 2000)
    cijk = [[[ (sym.diff(M[k,j], q[i]) + sym.diff(M[k,i], q[j]) - sym.diff(M[i,j], q[k]))/2
                for k in range(n)] for j in range(n)] for i in range(n)]

    # Derive the elements of the C matrix                
    ckj = [[sum([cijk[i][j][k]*q_dot[i] for i in range(n)])
...
class SymbolicMechanicalSystem(SimpleMechanicalSystem):
    def __init__(self, modelfile, parameters, q_initial=None, q_dot_initial=None, exogenous_input_function=None):    
        ...
        # Create the lambda functions, with the parameter values already substituted
        ...
        SimpleMechanicalSystem.__init__(self, M, F, C, G, Q, q_initial, q_dot_initial, exogenous_input_function)

        # If the model already contained a state feedback law, apply it
        if 'tau' in model:
            self.state_feedback = lambda x, time: model['tau'](self.get_coordinates(x)[0], # q
