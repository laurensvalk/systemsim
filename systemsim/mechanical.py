...
class SimpleMechanicalSystem(System):
    ...
    def __init__(self, M, F, C=None, G=None, Q=None,
                 q_initial=None, q_dot_initial=None,
                 exogenous_input_function=None):
        """Initialize"""
        # Store matrices
        self.M, self.F = M, F

        # Determine number of coordinates from shape of M in order to avoid the user having to supply it
        ...
        q_initial = q_initial if q_initial is not None else np.zeros(self.n)
        q_dot_initial = q_dot_initial if q_dot_initial is not None else np.zeros(self.n)
        ...
        # Store remaining system matrix generators or create them as zeros otherwise
        ...
        self.state_names = ['q_' + str(i) for i in range(self.n)] + ['q_dot' + str(i) for i in range(self.n)]
    ...
    def state_feedback(self, state, time=None):
        """Internal mechanical system feedback (none for now)"""
    ...
    def equations_of_motion(self, state, tau, time=None):
        ...
        Q = self.Q(q, q_dot)
        G = self.G(q)
        q_dotdot = np.linalg.solve(M, Q+F@tau.reshape(self.m,1)-G-C@q_dot.reshape(self.n,1))        
        return np.append(q_dot, q_dotdot).reshape(2*self.n)

# TODO DERIVE A Lagrangian and Hamiltonian variant or add way to access momenta, storage, supply etc
# Either from above, or derived from Hamiltonian sys...
