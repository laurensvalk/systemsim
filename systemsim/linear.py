...
class LTI(System):
    """LTI System"""
    def __init__(self, A, B, C, F=None, x_initial=None, exogenous_input_function=None):
        """Initialize"""
        # Store matrices
        ...
        assert A.shape[self.ROW] == A.shape[self.COL] == B.shape[self.ROW] == C.shape[self.COL]
        assert self.F.shape[self.ROW] == n_inputs and self.F.shape[self.COL] == n_states

        # Initialize System object
        System.__init__(self, n_states, n_inputs, n_outputs, x_initial, exogenous_input_function)

    def output(self, state, time):
        "LTI output without feedthrough."
        return self.C@state
